...
const settings = {
  ...
  store_description: 'Оригинальные кроссовки с доставкой по Узбекистану',
...
async function seed() {
  ...
  try {
    ...
    await client.query(
      'TRUNCATE order_items, orders, products, banners, categories RESTART IDENTITY CASCADE'
    )
    ...
    for (const b of banners) {
      await client.query(
        'INSERT INTO banners (image_url, title, subtitle, sort_order, active) VALUES ($1, $2, $3, $4, true)',
        [b.image_url, b.title, b.subtitle, b.sort_order]
      )
    }
